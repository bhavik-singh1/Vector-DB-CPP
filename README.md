...
Inside the `Your-OWN-AI` folder, you have two ways to build:

**Option A: Simple Build Script (Recommended)**
```powershell
./build.bat
```

**Option B: Professional Build (CMake)**
...
│   ├── Core/           ← VectorDB, DocumentDB, Types
│   ├── Ollama/         ← Ollama Client
│   └── Utils/          ← Distance, JSON, Text utilities
├── src/                ← Implementation files (.cpp)
├── main.cpp            ← API Server & Routing
├── httplib.h           ← Single-header HTTP library
├── index.html          ← Frontend UI
├── CMakeLists.txt      ← Professional Build Config
└── build.bat           ← Quick-start Build Script
```

### Architecture
The engine is now fully modular:
- **Algorithms**: Pure implementations of HNSW and KD-Tree, decoupled from the DB logic.
- **Core**: Unified interface for demo vectors and real document embeddings.
- **Ollama**: Clean REST client abstraction for AI model communication.
